...
$(document).ready(function onDocumentReady() {
  ...
  ui.projectionDisplayButtons.globe.click(
    setProjectionRenderMode.bind(null, "globe")
  );
  ui.projectionDisplayButtons.equalAreaMap.click(
    setProjectionRenderMode.bind(null, "equalAreaMap")
  );
  ui.projectionDisplayButtons.mercatorMap.click(
    setProjectionRenderMode.bind(null, "mercatorMap")
  );
...
SteppedAction.prototype.executeSubaction = function SteppedAction_executeSubaction(
  subaction,
  proportion,
  name
) {
...
function render() {
  ...
  var frameDuration =
    lastRenderFrameTime !== null
      ? Math.min((currentRenderFrameTime - lastRenderFrameTime) * 0.001, 0.1)
      : 0;
...
var mapProjections = {
  ...
  mercatorMap: function mercatorMap(longitude, latitude, z) {
    return new Vector3(
      longitude / Math.PI,
      Math.log(
        Math.tan(Math.PI / 4 + Math.max(-1.5, Math.min(latitude * 0.4, 1.5)))
      ) / 1.75,
      z
    );
  }
...
var mapProjectionsInverse = {
  ...
  mercatorMap: function mercatorMap(x, y) {
    return {
      longitude: x * Math.PI,
      latitude: (Math.atan(Math.exp(y * 1.75)) - Math.PI / 4) * 2.5
    };
  }
};
...
function renderMap() {
  ...
  if (project) {
    ...
    if (tileSelection !== null) {
      ...
      planet.renderData.surface.mapRenderObject.add(
        tileSelection.mapRenderObject
      );
    }
  }
}
...
function deselectTile() {
  if (tileSelection !== null) {
    ...
    if (tileSelection.mapRenderObject) {
      planet.renderData.surface.mapRenderObject.remove(
        tileSelection.mapRenderObject
      );
    }
    tileSelection = null;
...
function clickHandler(event) {
  if (planet) {
    ...
    if (projectionRenderMode === "globe") {
      ...
      var transformation = new THREE.Matrix4().makeRotationFromEuler(
        new THREE.Euler(cameraLatitude, cameraLongitude, 0, "ZYX")
      );
...
function projectMap(renderData, globeColorArrayKeys, project) {
  ...
  var transformation = new THREE.Matrix4().makeRotationFromEuler(
    new THREE.Euler(-cameraLatitude, -cameraLongitude, 0, "XYZ")
  );
  ...
  for (var i = 0; i < globeGeometry.vertices.length; ++i) {
    ...
    mapGeometry.vertices.push(
      project(
        longitude,
        latitude,
        Math.sqrt(xSquared + ySquared + zSquared) - 1
      )
    );
  }
  ...
  if (globeColorArrayKeys) {
    for (var i = 0; i < globeGeometry.faces.length; ++i) {
      ...
      mapGeometry.faces.push(
        new THREE.Face3(globeFace.a, globeFace.b, globeFace.c, globeFace.normal)
      );
    }
  } else {
    for (var i = 0; i < globeGeometry.faces.length; ++i) {
    ...
    addFace = function addFace(
      ...
      mapGeometry.faces.push(
        new THREE.Face3(
          ...
          vertexIndex1,
          vertexIndex2,
          globeGeometry.faces[faceIndex].normal
        )
      );
      for (var i = 0; i < globeColorArrayKeys.length; ++i) {
        var key = globeColorArrayKeys[i];
        renderData.mapColorArrays[key].push(
          renderData[globeColorArrayKeys[i]][faceIndex].slice()
        );
      }
    };
  } else {
    addFace = function addFace(
      ...
      mapGeometry.faces.push(
        new THREE.Face3(
          ...
          vertexIndex2,
          globeGeometry.faces[faceIndex].normal,
          globeGeometry.faces[faceIndex].vertexColors
        )
      );
    };
  }
    ...
    if (xWrap !== 0) {
      if (yWrap !== 0) {
        ...
        addFace(
          i,
          addVertex(
            p0.x + cornerWrap(xWrap01, xWrap02),
            p0.y + cornerWrap(xWrap01, xWrap02),
            p0.z
          ),
          addVertex(
            p1.x + cornerWrap(xWrap12, -xWrap01),
            p1.y + cornerWrap(xWrap12, -xWrap01),
            p1.z
          ),
          addVertex(
            p2.x + cornerWrap(-xWrap02, -xWrap12),
            p2.y + cornerWrap(-xWrap02, -xWrap12),
            p2.z
          )
        );
  ...
  if (renderData.mapRenderObject && renderData.mapRenderObject.parent)
    renderData.mapRenderObject.parent.remove(renderData.mapRenderObject);

  renderData.mapGeometry = mapGeometry;
  renderData.mapRenderObject = new THREE.Mesh(
    renderData.mapGeometry,
    renderData.mapMaterial
  );
}
  ...
  if (mode !== surfaceRenderMode || force === true) {
    ...
    if (projectionRenderMode === "globe") {
      ...
      if (mode === "terrain") colors = planet.renderData.surface.terrainColors;
        ...
        colors = planet.renderData.surface.plateColors;
      ...
      if (mode === "terrain")
        colors = planet.renderData.surface.mapColorArrays.terrainColors;
      else if (mode === "plates")
        colors = planet.renderData.surface.mapColorArrays.plateColors;
      else if (mode === "elevation")
        colors = planet.renderData.surface.mapColorArrays.elevationColors;
      else if (mode === "temperature")
        colors = planet.renderData.surface.mapColorArrays.temperatureColors;
      else if (mode === "moisture")
        colors = planet.renderData.surface.mapColorArrays.moistureColors;
      else return;
    }
...
function showHidePlateBoundaries(show) {
  ...
  if (renderPlateBoundaries) {
    ...
    planet.renderData.surface.mapRenderObject.add(
      planet.renderData.plateBoundaries.mapRenderObject
    );
  } else {
...
function showHidePlateMovements(show) {
  ...
  if (renderPlateMovements) {
    ...
    planet.renderData.surface.mapRenderObject.add(
      planet.renderData.plateMovements.mapRenderObject
    );
  } else {
    planet.renderData.surface.renderObject.remove(
      planet.renderData.plateMovements.renderObject
...
function showHideAirCurrents(show) {
  ...
  if (renderAirCurrents) {
    ...
    planet.renderData.surface.mapRenderObject.add(
      planet.renderData.airCurrents.mapRenderObject
    );
  } else {
    planet.renderData.surface.renderObject.remove(
      planet.renderData.airCurrents.renderObject
  ...
  var movement = this.driftAxis
    ...
    .setLength(
      this.driftRate *
        position
          .clone()
          .projectOnVector(this.driftAxis)
          .distanceTo(position)
    );
  movement.add(
    this.root.position
...
SpatialPartition.prototype.intersectRay = function SpatialPartition_intersectRay(
  ray
) {
